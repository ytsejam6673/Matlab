...
%DOUBLE_PENDULUM_SIMULATION Plots double pendulum until user responds
%
%   Simulates the oscilations of the double simple pendulum until the user
%   presses a key or clickes in the figure. If any of input parameters is
%   not given, it will be assigned a default value.
%
...
sint = sin(p);
cost = cos(p);
patch(xm1+r1*cost,ym1+r1*sint,0,'EdgeColor','b','FaceColor','b') % 진자1
patch(xm2+r2*cost,ym2+r2*sint,0,'EdgeColor','b','FaceColor','b') % 진자2
